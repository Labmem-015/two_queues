...
struct Task {
  ...
  Task<T> &operator=(Task<T> &&other) {
    ...
    if (coro_handle) {
      coro_handle.destroy(); // this destroys the frame
                             // YOU CANNOT RESUME DESTROYED FRAMES
                             // suspend_always because you allow moves
                             // you can't not suspend or else UB
    }
  ...
  T get_value() { return coro_handle.promise().value; }
...
struct Awaitable {
  ...
  Awaiter operator co_await() { return m_awaiter; }
